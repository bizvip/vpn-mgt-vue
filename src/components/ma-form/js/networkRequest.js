...
export const requestDict = (url, method, params, data, timeout = 10 * 1000) => request({
    url,
    method,
    params,
    data,
...
export const handlerDictProps = (item, tmpArr) => {
    ...
    if (allowCoverComponent.includes(item.formType)) {
        data = tmpArr.map(dicItem => {
            ...
            let disabled = (typeof dicItem['disabled'] == 'undefined') ? false : (dicItem['disabled'] === true)
            let indeterminate = (typeof dicItem['indeterminate'] == 'undefined') ? false : (dicItem['indeterminate'] === true)
            let value
...
export const handlerCascader = async (val, column, columns, dictList, formModel, clearData = true) => {
    if (column.cascaderItem && isArray(column.cascaderItem)) {
        column.cascaderItem.map(async name => {
            ...
            await requestCascaderData(val, dict, dictList, name)
        })
    }
}