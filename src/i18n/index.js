...
const getLanguage = () => {
    const loadFile = () => {
        if (setting.language === 'zh_CN') {
            return import.meta.glob('./zh_CN/*.js')
        } else if (setting.language === 'en') {
            return import.meta.glob('./en/*.js')
        }
    }
...
const i18n = createI18n({
    locale: setting.language, legacy: false, globalInjection: true, fallbackLocale: 'zh_CN', messages: getLanguage()
})
